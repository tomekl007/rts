...
void sig_handler(int signo)
{
	if(signo == SIGINT)
	{
		puts("--- sygnal SIGINT --- zabijanie dzieci p1");
		kill(pid1, SIGINT);
		kill(pid2, SIGINT);
...
int main(int argc, char *argv[])
{
	...
	const char* taskiCzytajace[4]= {"0","1","2","3"};
	const char* taskiPiszace[3]={"0","1","2"};

	if( (pid1 = fork()) == 0)
		execl("pC2", taskiCzytajace[0],taskiCzytajace[1],taskiCzytajace[2],taskiCzytajace[3], NULL);

	if( (pid2 = fork()) == 0)
	    execl("pP", taskiPiszace[0],taskiPiszace[1],taskiPiszace[2], NULL);
	else
